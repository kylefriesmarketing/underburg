...
import {WorldDioramas} from './world-dioramas.js';
const TAU=Math.PI*2;
...
export class OceanWorld {
 ...
 clearRegion(){
  ...
  for(const o of this.landmarks){this.root.remove(o);this.view.release(o);}this.landmarks=[];this.bubbles=[];this.dioramas.clear();this.clearBoundary();this.lastCull.set(Infinity,Infinity);
 }
 setRegion(stage,seed,landmarkDefs,menu=false){
  ...
  for(const d of landmarks){const presentation=window.UBWorld.landmarkPresentation(d,stage);const o=this.view.clone(presentation.model,true);o.name=d.id;o.userData.landmarkModel=presentation.model;o.userData.solidRadius=d.solidRadius||0;o.position.set(d.x*.1,height(d.x*.1,d.y*.1,stage),d.y*.1);o.rotation.y=d.rotation||0;o.scale.setScalar(presentation.scale);this.root.add(o);this.landmarks.push(o);if(d.model.includes('basalt')||d.model.includes('pipeline'))this.bubbles.push({x:d.x*.1,z:d.y*.1});}
  ...
  this.dioramas.build(stage,seed,landmarks,sites);
 ...
 get stats(){return {dioramas:this.dioramas.stats,expedition:this.expeditionView.stats,landmarks:this.landmarks.length,anchoredDecor:this.deco.length,visibleInstances:this.visibleDecor,rootChildren:this.root.children.length,cullExtents:this.extents,floorHalfExtent:650,boundaryTriangles:this.boundaryTriangles,edgeClearance:this.edgeClearance,stage:this.stage};}
}
