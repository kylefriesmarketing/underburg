...
const blueprint=(id,name,english,desc,source,price=0)=>({id:'weapon-'+id,type:'weapon',weapon:id,name,english,desc,model:'module-'+id,source,price});
const CATALOG=[
 ...
 blueprint('rail','Sternlanze','Rail weapon blueprint','Unlocks the rail weapon for starting loadouts and future module drafts.','silver',100),
 blueprint('flame','Glutstrom','Thermal weapon blueprint','Unlocks the flame weapon for starting loadouts and future module drafts.','feuerprobe'),
 blueprint('sonic','Schallbrecher','Sonic weapon blueprint','Unlocks the sonic weapon for starting loadouts and future module drafts.','tiefensucher'),
 blueprint('vortex','Strudelkern','Vortex weapon blueprint','Unlocks the vortex weapon for starting loadouts and future module drafts.','drei-meere')
];
...
function freshHarbor(){return {version:1,owned:[...FREE_ITEMS],equipped:{paint:'paint-original',emblem:'emblem-anchor',fitting:'fitting-balanced'},achievements:[],settled:[],stats:{runs:0,wins:0,kills:0,gold:0,chests:0,upgrades:0,abilities:0,bosses:0,bestLevel:0,highestRarity:'COMMON',landmarks:[],regions:[],missions:[]}};}
function sanitizeHarbor(raw){
 const state=freshHarbor();if(!raw||raw.version!==1||typeof raw!=='object')return state;
 ...
 const earned=state.achievements.map(id=>ACHIEVEMENT_BY_ID[id].reward);
 const bought=Array.isArray(raw.owned)?raw.owned.filter(id=>own(ITEMS,id)&&ITEMS[id].source==='silver'):[];
...
function purchase(state,meta,itemId){
 ...
 if(item.source!=='silver'||safe.owned.includes(itemId)||!Number.isFinite(meta.silver)||meta.silver<item.price)return false;
...
function awardAchievements(state,campaign){
 const awarded=[];for(const progress of achievementProgress(state,campaign)){if(progress.awarded||progress.value<progress.target)continue;state.achievements.push(progress.id);if(!state.owned.includes(progress.reward))state.owned.push(progress.reward);awarded.push({...progress,awarded:true,rewardName:progress.rewardItem.name,rewardText:progress.rewardItem.name+' unlocked'});}return awarded;
}
...
const api={CATALOG,ITEMS,ACHIEVEMENTS,ACHIEVEMENT_BY_ID,BASE_WEAPONS,RARITY_ORDER,freshHarbor,sanitizeHarbor,purchase,equip,recordResult,syncProgress,getLoadout,achievementProgress,evaluate:achievementProgress};scope.UBHarbor=api;if(typeof module!=='undefined')module.exports=api;
