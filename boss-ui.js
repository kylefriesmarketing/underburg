...
function mount(){
 ...
 for(let i=0;i<3;i++){const row=document.createElement('div');row.className='boss-contact';row.dataset.bossContact=i;row.innerHTML='<div><span></span><b></b></div><div class="bar"><i></i></div>';rows.append(row);}
...
export function syncBossHud(g,fmt){
 ...
 timeline.hidden=progressLabel.hidden=!mission;rows.hidden=!mission;
 ...
 for(let i=0;i<3;i++){const el=timeline.children[i],wave=g.mission.bossWaves[i],defeated=g.bossKillWaves.includes(i),live=active.find(e=>e.bossWave===i);
  ...
  const row=rows.children[i];row.hidden=!live;if(live){row.querySelector('span').textContent=(i+1)+' / '+live.bossName;row.querySelector('b').textContent=Math.ceil(live.hp/live.max*100)+'%';row.querySelector('i').style.width=Math.max(0,live.hp/live.max*100)+'%';row.title=live.bossName+' · '+Math.ceil(live.hp)+' / '+Math.round(live.max)+' hull';}
 }
