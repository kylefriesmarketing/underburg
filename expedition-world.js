...
export class ExpeditionWorld {
 ...
 build(game){
  ...
  for(const site of game.sites||[]){
   ...
   for(let i=0;i<24;i++){const a=i/24*TAU,r=site.kind==='bastion'?25:19,px=x+Math.cos(a)*r,pz=z+Math.sin(a)*r;paving.push({x:px,y:floor(px,pz,game.stage)+.12,z:pz,a:-a});}
  }
  ...
  for(const [index,landmark]of (game.world?.landmarks||[]).entries()){
   ...
   for(let row=0;row<3;row++)for(let i=0;i<12;i++){const a=row*TAU/3+index*.73,r=17+i*2.7,px=x+Math.cos(a)*r,pz=z+Math.sin(a)*r;paving.push({x:px,y:floor(px,pz,game.stage)+.16,z:pz,a:-a});}
   for(let i=0;i<7;i++){const a=i/7*TAU+index*.7,r=25+hash(i+index*37)*15,px=x+Math.cos(a)*r,pz=z+Math.sin(a)*r;const ruin=this.view.clone(i%3===0?'world-arch':i%3===1?'colony':'wreck',true);ruin.scale.setScalar(i%3===0?.27:.3);ruin.rotation.set(i%3===0?.65:0,a,i%3===0?.2:0);ruin.position.set(px,floor(px,pz,game.stage)-.3,pz);this.root.add(ruin);this.ruins.push(ruin);}
  }
  const streets=new THREE.InstancedMesh(new THREE.BoxGeometry(2.4,.16,1.3),new THREE.MeshStandardMaterial({color:stone,roughness:.97}),paving.length);streets.name='sunken-paved-streets';for(let i=0;i<paving.length;i++){const p=paving[i];this.fishDummy.position.set(p.x,p.y,p.z);this.fishDummy.rotation.set(0,p.a,0);this.fishDummy.scale.setScalar(1);this.fishDummy.updateMatrix();streets.setMatrixAt(i,this.fishDummy.matrix);}streets.receiveShadow=true;streets.computeBoundingSphere();this.root.add(streets);
