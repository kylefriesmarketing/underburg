...
export class ShipAssembly{
 ...
 ensureModule(key,index,animate){
  if(this.modules.has(key))return this.modules.get(key);
  const socket=new THREE.Group(),weapon=this.tint(this.view.clone('module-'+key,this.shadow)),mount=MOUNTS[index%MOUNTS.length];socket.scale.setScalar(index>=4?.7:.83);socket.rotation.y=index===1?Math.PI/2:index===2?-Math.PI/2:index===3?Math.PI:0;socket.userData.module=key;socket.add(weapon);this.body.add(socket);this.dock(socket,new THREE.Vector3(...mount),animate);
  const entry={key,index,socket,weapon,recoil:0,kick:0,baseScale:1,flash:0,aim:0};this.modules.set(key,entry);return entry;
 }
 sync(tier,keys,states=null,animate=false,showcase=false){
  ...
  keys.forEach((key,index)=>{
   const entry=this.ensureModule(key,index,animate),m=data[key]||{},purchases=m.purchases||0;
  ...
  if(this.playerHull){
   ...
   if(tier>=4)add('citadel-crown','growth-crown',[-1.1,4.38,0],slim?.73:wide?1.02:.9,0,animate);
  }
 ...
 shot(key,active=false,angle=null){const entry=this.modules.get(key);if(!entry)return null;entry.kick=Math.min(.36,Math.max(entry.kick,active?.3:.16));entry.recoil=1;entry.flash=.11;if(Number.isFinite(angle)){entry.aim=-angle-this.root.rotation.y-entry.socket.rotation.y;entry.weapon.rotation.y=entry.aim;}const point=new THREE.Vector3(1.1,.8,0);entry.weapon.updateWorldMatrix(true,false);entry.weapon.localToWorld(point);return point;}
 ...
 get stats(){return{parts:this.parts.size,partIds:[...this.parts.keys()],modules:this.modules.size,docking:this.animations.length,docked:this.docked,bank:this.bank,pitch:this.pitch,thrust:this.thrust,recoil:[...this.modules.values()].reduce((n,m)=>Math.max(n,m.recoil),0),rotors:this.rotors.length};}
}
